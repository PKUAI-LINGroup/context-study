...
class TrmDecoderBlock(nn.Module):
...
class TrmDecoderModule(GPT2PreTrainedModel):
    def __init__(self, config, n_utterance_positions):
        super().__init__(config)

        self.wte = nn.Embedding(config.vocab_size, config.n_embd)
        self.wpe = nn.Embedding(config.n_positions, config.n_embd)
        self.upe = nn.Embedding(n_utterance_positions, config.n_embd)
        self.drop = nn.Dropout(config.embd_pdrop)
        self.h = nn.ModuleList([TrmDecoderBlock(config.n_ctx, config, scale=True) for _ in range(config.n_layer)])
