...
class DataPreprocessor:
    ...
    def process_data(self, input_file_path, output_file_path):
        ...
        print(f"After parsing, we get {len(multi_turn_data)} dialogs.")
        split_data = self.split_data(data=multi_turn_data)
        print(f"After splitting, we get {len(split_data)} samples.")
    ...
    def split_data(self, data):
        ...
        for dialog in tqdm(data):
            for utter_idx in range(1, len(dialog['dialog'])):
...
if __name__ == "__main__":
    ...
    for phase in ["train", "validation", "test"]:
        print(f"Now processing {phase} data:")
